...
fn test_basic_rawptr() {
    let mut m = ds_rs::linked_list::rawptr::LinkedList::<Box<_>>::new();
    assert_eq!(m.pop_front(), None);
    assert_eq!(m.pop_back(), None);
    assert_eq!(m.pop_front(), None);
    m.push_front(Box::new(1));
...
#[test]
fn test_empty_rawptr() {
    use ds_rs::linked_list::rawptr::LinkedList;
    let list: LinkedList<i32> = LinkedList::new();
    assert!(list.is_empty());
    assert_eq!(list.len(), 0);
}
...
#[test]
fn test_push_back_rawptr() {
    let mut list = ds_rs::linked_list::rawptr::LinkedList::new();
    list.push_back(1);
    assert!(!list.is_empty());
    assert_eq!(list.len(), 1);
    assert_eq!(list.front(), Some(&1));
